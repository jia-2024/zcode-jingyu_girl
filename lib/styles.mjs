...
export function resolveArt(styleId, proportionId, stateName, fallbackMap) {
  ...
  const isSticker = style.id === 'sticker'
  let key = stateName
  ...
  let rel = states[key]
  if (isSticker && (!rel || rel === '__sticker__')) rel = 'forms/classic.png' // 贴纸画风：身体固定原版鲸鱼
  const imageUrl = rel ? '/api/assets/characters/deepseek/' + rel : null
